...
class ControleChat{
    public function GravarMensagem($dados){
        ...
        try{
            ...
            }else if($mensagem == "/apagaronlines"){
                $retorno = true;
                fwrite(fopen("../conteudo/Online.txt","w"),"");
            }else{
                $retorno = true;
                fwrite(fopen("../conteudo/Chat.txt","a"),$resultado);
            }
        }catch(Exception $e){
            ...
            foreach($dados->getFoto() as $arquivo){
                ...
                if(strstr($permitidos,$extensao) AND $tamanho<=2097152){
                    if(move_uploaded_file($imagem, "../fotos/".$nome)){
                        ...
                        if(fwrite(fopen("../conteudo/Chat.txt","a"),$resultado)){
                            $retorno = true;
                        }
                    }
                }
